...
class LinearModel:
    ...
    def activation(self, x):
        ...
        """
        if not type(x) == float:
            raise ValueError('Input type is' + str(type(x)))
        # sigmoid function
        return 1/(1 + np.exp(-x))
    ...
    def forward(self, inputs):
        ...
        """
        return self.activation(float((np.append(inputs, 1) * self.weights).sum()))
    ...
    def loss(prediction, label):
        ...
        """
        delta = 1e-7
        return -label * np.log(prediction + delta) - (1 - label) * np.log(1 - prediction + delta)
